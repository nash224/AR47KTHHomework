...
public:
	...
	// => 기존 객체의 멤버변수를 그대로 복사하여 새로운 객체에 할당하는 것 (주소까지 복사된다)
	// 정적 메모리를 사용할 때는 메모리 누수가 나지 않음

	// But 클래스가 동적으로 할당된 메모리를 가지고 있고, 얕은 복사를 새로운 객체에 할당한 경우
	// 메모리 누수가 날 수 있음. (주소까지 같이 복사되기 때문에 메모리를 잃어버림)
	//GameEngine& operator=(const GameEngine& _Other) = delete; // 해석) 이 복사, 대입 연산자 함수는 실행되지 않음
	...
	GameEngine& operator=(const GameEngine& _Other) // 사용할 수 있게 delete를 제거해줌
	{
		...
		// => 값을 변경하거나 함수를 호출할 수 있는 객체의 참조를 반환한다
	...
	// ArrCount 변수의 값을 리턴해주기 위해 자료형을 size_t(int)로 반환 
	size_t Count()
	{
		return ArrCount;
	}
